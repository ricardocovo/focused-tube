---
description: 'Implements a GitHub-native feature spec end-to-end. Reads the parent feature issue and related story issues generated by gh-feature-spec-generator, builds a sequenced implementation plan with explicit dependencies, implements each story (using sub-agents when appropriate), validates the result, and updates README/ARCHITECTURE if needed.'
...
Primary mode: GitHub issue-driven implementation using GitHub CLI (`gh`) and the `gh-cli` skill.
Compatibility mode: local spec file implementation when issue inputs are not available.

## Instructions

Use the `gh-cli` skill as the canonical command reference for all GitHub issue operations.
